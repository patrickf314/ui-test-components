...
/**
 * Unit test for {@link PlaywrightImageComponentAssert}
 */
class PlaywrightImageComponentAssertTest {
    ...
    @Test
    void testLoadedSuccessfull() {
        // language=HTML
        try (var context = mockPage("""
                <img src='%s' alt="Some alternate text"/>
                """.formatted(IMAGE_SRC))) {
            var imageAssert = new PlaywrightImageComponentAssert(context.component("img", PlaywrightImageComponent::new));

            assertThatCode(imageAssert::isLoaded).doesNotThrowAnyException();
            assertThatThrownBy(imageAssert::hasFailedLoading)
    ...
    void testFailedLoading() {
        // language=HTML
        try (var context = mockPage("""
                <img src='/does/not/exist.jpg' alt="Some alternate text"/>
                """)) {
            var imageAssert = new PlaywrightImageComponentAssert(context.component("img", PlaywrightImageComponent::new));

            assertThatCode(imageAssert::hasFailedLoading).doesNotThrowAnyException();
            assertThatThrownBy(imageAssert::isLoaded)
                    .isInstanceOf(AssertionError.class);
        }
    }
}